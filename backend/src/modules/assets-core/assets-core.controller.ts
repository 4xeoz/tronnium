...
import type { CpeInput } from "./asset.types";
import { ok, err } from "../../lib/response-helpers";
import { toCpeCandidate } from "./assets-core.helpers";
    ...
    try {
        ...
        if (!name || typeof name !== "string" || name.trim().length === 0) {
            return res.status(400).json(
                err("INVALID_INPUT", "name is required and must be a non-empty string")
            );
        }
        ...
        if (cpes == undefined) {
            return res.status(400).json(
                err("INVALID_INPUT", "cpes field is required")
            );
        }

        // reject if cpes is provided but is not an array of at least one object 
        if (cpes !== undefined) {
            if (!Array.isArray(cpes) || cpes.length === 0 || !cpes.every(cpe => typeof cpe === 'object' && cpe !== null)) {
                return res.status(400).json(
                    err("INVALID_INPUT", "cpes must be an array of objects")
                );
            }
        }
